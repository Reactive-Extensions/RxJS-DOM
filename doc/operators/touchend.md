### `Rx.DOM.touchend(element, [selector], [useCapture])`
[&#x24C8;](https://github.com/Reactive-Extensions/RxJS-DOM/blob/master/src/events/events.js "View in source")

Creates an observable sequence by adding an event listener to the matching DOMElement or DOMNodeList for the `touchend` event.  This method is only available when Touch Events are supported on the browser.
...
#### Returns
*(`Observable`)*: An observable sequence of events from the specified element and the `touchend` event.
...
var source = Rx.DOM.touchend(input);
