### `Rx.DOM.pointerout(element, [selector], [useCapture])`
[&#x24C8;](https://github.com/Reactive-Extensions/RxJS-DOM/blob/master/src/events/events.js "View in source")

Creates an observable sequence by adding an event listener to the matching DOMElement or DOMNodeList for the `pointerout` event.  This method is only available when Pointer Events are supported on the browser.
...
#### Returns
*(`Observable`)*: An observable sequence of events from the specified element and the `pointerout` event.
...
var source = Rx.DOM.pointerout(input);
